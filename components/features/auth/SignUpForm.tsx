...
export function SignUpForm() {
  ...
  return (
    <div className="w-full max-w-md p-8 space-y-8 bg-white/10 backdrop-blur-md rounded-xl shadow-lg">
      <h2 className="text-2xl font-bold text-center text-white">新規登録</h2>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <div>
          <label htmlFor="name" className="block mb-2 text-sm font-medium text-white">名前</label>
          <input id="name" {...form.register("name")} className="w-full px-3 py-2 text-white bg-white/20 rounded-md focus:outline-none focus:ring-2 focus:ring-white/50" />
        </div>
        <div>
